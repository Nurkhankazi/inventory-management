...
function PurchasePay() {
    const {id} = useParams();
    const [inputs, setInputs] = useState({check_date:'',bank_name:'',check_number:''});
    const [purchaseData, setPurchaseData] = useState({check_date:'',bank_name:'',check_number:''});
    ...
    const handleSubmit = async(e) => {
        ...
        try{
            let apiurl=`/purchase/payment/${id}`;//api from laravel
            ...
            <div className="page-title">
                <div className="row">
                    <div className="col-12 col-md-12 order-md-1 order-last">
                        <br/><h1 style={{textAlign: 'center'}}>Purchase Payment</h1>
                    </div>
                </div>
            </div>
                                        ...
                                        <div className="form-body">
                                            <div className='row'> 
                                                <div className="col-6 offset-6 d-flex justify-content-end">
                                                    {purchaseData.gtotal > 0 &&
                                                        <h3>Due Amount: {purchaseData.gtotal - purchaseData.payment_sum_amount}</h3>
                                                    }
                                                </div>
                                            </div>
                                            {/* Payment type */}
